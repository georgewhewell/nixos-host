...
let
  cfg = config.services.kubernetes_15;
...
in {
  ...
  options.services.kubernetes_15 = {
    roles = mkOption {
      description = ''
  ...
  config = mkMerge [
    ...
    (mkIf cfg.kubelet.enable {
      systemd.services.kubelet = {
        ...
        path = with pkgs; [ gitMinimal openssh docker utillinux iproute ethtool thin-provisioning-tools iptables zfs ];
        preStart = ''
      ...
      services.kubernetes_15.kubelet.cni.packages = [pkgs.cni];
    })
    ...
    (mkIf (any (el: el == "master") cfg.roles) {
      ...
      services.kubernetes_15.kubelet.enable = mkDefault true;
      services.kubernetes_15.kubelet.allowPrivileged = mkDefault true;
    ...
    (mkIf (any (el: el == "node") cfg.roles) {
      ...
      services.kubernetes_15.kubelet.enable = mkDefault true;
      services.kubernetes_15.proxy.enable = mkDefault true;
      services.kubernetes_15.dns.enable = mkDefault true;
    })
